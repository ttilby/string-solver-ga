...
const randomSeed = (length) => {
  ...
  let seed = '';
  while (l--) {
    seed += String.fromCharCode(Math.floor(Math.random() * (126 - 32 + 1)) + 32);
  }
  return seed;
}
...
const calcFitness = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (a === b) {
      console.log(`a: (${String.fromCharCode(a)}) ${a} b: ${b} (${String.fromCharCode(b)}) score: ${1}`);
      score += 1;
    } else if (a < b) {
      console.log(`a: (${String.fromCharCode(a)}) ${a} b: ${b} (${String.fromCharCode(b)}) (a-b) score: ${a - b}`);
      score += (a - b);
    } else {
      console.log(`a: (${String.fromCharCode(a)}) ${a} b: ${b} (${String.fromCharCode(b)}) (b-a) score: ${b - a}`);
      score += (b - a);
    }
...
while (!done(population)) {
  ...
  console.log(`generation: ${generation} -- fitness: ${sorted[0].fitness} : ${sorted[0].value}`);
